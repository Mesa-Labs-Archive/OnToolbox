...
class SeslAdapterHelper implements SeslOpReorderer.Callback {
    ...
    private void dispatchAndUpdateViewHolders(UpdateOp op) {
        ...
        if (DEBUG) {
            LogUtils.d(TAG, "dispatch (pre)" + op);
            LogUtils.d(TAG, "postponed state before:");
            for (UpdateOp updateOp : mPostponedList) {
                LogUtils.d(TAG, updateOp.toString());
            }
            LogUtils.d(TAG, "----");
        }
        ...
        if (DEBUG) {
            LogUtils.d(TAG, "pos:" + op.positionStart + ",updatedPos:" + tmpStart);
        }
        int tmpCnt = 1;
        ...
        for (int p = 1; p < op.itemCount; p++) {
            ...
            if (DEBUG) {
                LogUtils.d(TAG, "pos:" + pos + ",updatedPos:" + updatedPos);
            }
            ...
            if (continuous) {
                ...
                if (DEBUG) {
                    LogUtils.d(TAG, "need to dispatch separately " + tmp);
                }
        ...
        if (tmpCnt > 0) {
            ...
            if (DEBUG) {
                LogUtils.d(TAG, "dispatching:" + tmp);
            }
            dispatchFirstPassAndUpdateViewHolders(tmp, offsetPositionForPartial);
            recycleUpdateOp(tmp);
        }
        if (DEBUG) {
            LogUtils.d(TAG, "post dispatch");
            LogUtils.d(TAG, "postponed state after:");
            for (UpdateOp updateOp : mPostponedList) {
                LogUtils.d(TAG, updateOp.toString());
            }
            LogUtils.d(TAG, "----");
    ...
    private int updatePositionWithPostponed(int pos, int cmd) {
        ...
        for (int i = count - 1; i >= 0; i--) {
            ...
            if (DEBUG) {
                LogUtils.d(TAG, "dispath (step" + i + ")");
                LogUtils.d(TAG, "postponed state:" + i + ", pos:" + pos);
                for (UpdateOp updateOp : mPostponedList) {
                    LogUtils.d(TAG, updateOp.toString());
                }
                LogUtils.d(TAG, "----");
