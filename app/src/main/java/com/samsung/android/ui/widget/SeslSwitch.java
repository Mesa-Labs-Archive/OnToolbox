...
public class SeslSwitch extends CompoundButton {
    ...
    private static final Property<SeslSwitch, Float> THUMB_POS =
            new Property<SeslSwitch, Float>(Float.class, "thumbPos") {
                @Override
                public Float get(SeslSwitch object) {
                    return object.mThumbPosition;
                }

                @Override
                public void set(SeslSwitch object, Float value) {
    ...
    public SeslSwitch(Context context) {
        this(context, null);
    }

    public SeslSwitch(Context context, AttributeSet attrs) {
        this(context, attrs, R.attr.switchStyle);
    }

    public SeslSwitch(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        final TintTypedArray a = TintTypedArray.obtainStyledAttributes(context, attrs, R.styleable.SeslSwitch, defStyleAttr, 0);
        mThumbDrawable = a.getDrawable(R.styleable.SeslSwitch_android_thumb);
        if (mThumbDrawable != null) {
            mThumbDrawable.setCallback(this);
        }
        mTrackDrawable = a.getDrawable(R.styleable.SeslSwitch_track);
        ...
        mTextOn = a.getText(R.styleable.SeslSwitch_android_textOn);
        mTextOff = a.getText(R.styleable.SeslSwitch_android_textOff);
        mShowText = a.getBoolean(R.styleable.SeslSwitch_showText, true);
        mThumbTextPadding = a.getDimensionPixelSize(R.styleable.SeslSwitch_thumbTextPadding, 0);
        mSwitchMinWidth = a.getDimensionPixelSize(R.styleable.SeslSwitch_switchMinWidth, 0);
        mSwitchPadding = a.getDimensionPixelSize(R.styleable.SeslSwitch_switchPadding, 0);
        mSplitTrack = a.getBoolean(R.styleable.SeslSwitch_splitTrack, false);

        ColorStateList thumbTintList = a.getColorStateList(R.styleable.SeslSwitch_thumbTint);
        if (thumbTintList != null) {
            mThumbTintList = thumbTintList;
            mHasThumbTint = true;
        }
        PorterDuff.Mode thumbTintMode = DrawableUtils.parseTintMode(a.getInt(R.styleable.SeslSwitch_thumbTintMode, -1), null);
        ...
        ColorStateList trackTintList = a.getColorStateList(R.styleable.SeslSwitch_trackTint);
        if (trackTintList != null) {
            mTrackTintList = trackTintList;
            mHasTrackTint = true;
        }
        PorterDuff.Mode trackTintMode = DrawableUtils.parseTintMode(a.getInt(R.styleable.SeslSwitch_trackTintMode, -1), null);
        ...
        final int appearance = a.getResourceId(R.styleable.SeslSwitch_switchTextAppearance, 0);
        if (appearance != 0) {
            ...
            case MotionEvent.ACTION_CANCEL: {
                if (mTouchMode == TOUCH_MODE_DRAGGING) {
                    if (Build.VERSION.SDK_INT >= 28 && action == MotionEvent.ACTION_UP && (mTouchMode == TOUCH_MODE_DRAGGING || mTouchMode == TOUCH_MODE_DOWN)) {
                        performHapticFeedback((int) Utils.genericInvokeMethod(HapticFeedbackConstants.class, Build.VERSION.SDK_INT >= 29 ? "hidden_semGetVibrationIndex" : "semGetVibrationIndex", 27));
                    }
                    stopDrag(ev);
    ...
    private void animateThumbToCheckedState(final boolean newCheckedState) {
        ...
        mPositionAnimator.setInterpolator(SeslElasticInterpolator.ELASTIC_50);
    ...
    public void seslSetTrackStrokeColor(int color) {
        // WTF Sammy, was all those casts necessary?
