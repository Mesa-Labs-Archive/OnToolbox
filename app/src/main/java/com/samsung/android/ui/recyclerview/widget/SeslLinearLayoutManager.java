...
public class SeslLinearLayoutManager extends SeslRecyclerView.LayoutManager implements ItemTouchHelper.ViewDropHandler, SeslRecyclerView.SmoothScroller.ScrollVectorProvider {
    ...
    public void onRestoreInstanceState(Parcelable state) {
        if (state instanceof SavedState) {
            ...
            requestLayout();
            if (DEBUG) {
                LogUtils.d(TAG, "loaded saved state");
            }
        } else if (DEBUG) {
            LogUtils.d(TAG, "invalid saved state class");
        }
    }
    ...
    public void smoothScrollToPosition(SeslRecyclerView recyclerView, SeslRecyclerView.State state, int position) {
        ...
        LogUtils.d(TAG, "SS pos to : " + position);
    }
    ...
    public void onLayoutChildren(SeslRecyclerView.Recycler recycler, SeslRecyclerView.State state) {
        if (DEBUG) {
            LogUtils.d(TAG, "is pre layout:" + state.isPreLayout());
        }
        ...
        if (DEBUG) {
            LogUtils.d(TAG, "Anchor info:" + mAnchorInfo);
        }
    ...
    private void layoutForPredictiveAnimations(SeslRecyclerView.Recycler recycler, SeslRecyclerView.State state, int startOffset, int endOffset) {
        ...
        if (DEBUG) {
            LogUtils.d(TAG, "for unused scrap, decided to add " + scrapExtraStart + " towards start and " + scrapExtraEnd + " towards end");
        }
    ...
    private void updateAnchorInfoForLayout(SeslRecyclerView.Recycler recycler, SeslRecyclerView.State state, AnchorInfo anchorInfo) {
        if (updateAnchorFromPendingData(state, anchorInfo)) {
            if (DEBUG) {
                LogUtils.d(TAG, "updated anchor info from pending information");
            }
            return;
        }
        ...
        if (updateAnchorFromChildren(recycler, state, anchorInfo)) {
            if (DEBUG) {
                LogUtils.d(TAG, "updated anchor info from existing children");
            }
            return;
        }
        if (DEBUG) {
            LogUtils.d(TAG, "deciding anchor info for fresh state");
        }
    ...
    private boolean updateAnchorFromPendingData(SeslRecyclerView.State state, AnchorInfo anchorInfo) {
        ...
        if (mPendingScrollPosition < 0 || mPendingScrollPosition >= state.getItemCount()) {
            ...
            mPendingScrollPositionOffset = INVALID_OFFSET;
            if (DEBUG) {
                LogUtils.e(TAG, "ignoring invalid scroll position " + mPendingScrollPosition);
            }
            return false;
        }
    ...
    public void scrollToPosition(int position) {
        ...
        SeslRecyclerView recyclerView = mRecyclerView;
    ...
    int scrollBy(int dy, SeslRecyclerView.Recycler recycler, SeslRecyclerView.State state) {
        ...
        if (consumed < 0) {
            if (DEBUG) {
                LogUtils.d(TAG, "Don't have any more elements to scroll");
            }
            return 0;
        }
        ...
        if (DEBUG) {
            LogUtils.d(TAG, "scroll req: " + dy + " scrolled: " + scrolled);
        }
    ...
    private void recycleChildren(SeslRecyclerView.Recycler recycler, int startIndex, int endIndex) {
        ...
        if (DEBUG) {
            LogUtils.d(TAG, "Recycling " + Math.abs(startIndex - endIndex) + " items");
        }
    ...
    private void recycleViewsFromStart(SeslRecyclerView.Recycler recycler, int dt) {
        if (dt < 0) {
            if (DEBUG) {
                LogUtils.d(TAG, "Called recycle from start with a negative value. This might happen" + " during layout changes but may be sign of a bug");
            }
            return;
        }
        final int limit = dt;
    ...
    private void recycleViewsFromEnd(SeslRecyclerView.Recycler recycler, int dt) {
        ...
        if (dt < 0) {
            if (DEBUG) {
                LogUtils.d(TAG, "Called recycle from end with a negative value. This might happen" + " during layout changes but may be sign of a bug");
            }
            return;
        }
    ...
    void layoutChunk(SeslRecyclerView.Recycler recycler, SeslRecyclerView.State state, LayoutState layoutState, LayoutChunkResult result) {
        ...
        if (DEBUG) {
            LogUtils.d(TAG, "laid out child at position " + getPosition(view) + ", with l:" + (left + params.leftMargin) + ", t:" + (top + params.topMargin) + ", r:" + (right - params.rightMargin) + ", b:" + (bottom - params.bottomMargin));
        }
    ...
    int convertFocusDirectionToLayoutDirection(int focusDirection) {
        switch (focusDirection) {
            ...
            default:
                if (DEBUG) {
                    LogUtils.d(TAG, "Unknown focus request:" + focusDirection);
                }
    ...
    private void logChildren() {
        LogUtils.d(TAG, "internal representation of views on the screen");
        for (int i = 0; i < getChildCount(); i++) {
            View child = getChildAt(i);
            LogUtils.d(TAG, "item " + getPosition(child) + ", coord:" + mOrientationHelper.getDecoratedStart(child));
        }
        LogUtils.d(TAG, "==============");
    }

    public void validateChildOrder() {
        LogUtils.d(TAG, "validating child count " + getChildCount());
        if (getChildCount() < 1) {
            return;
        }
    ...
    static class LayoutState {
        ...
        void log() {
            LogUtils.d(TAG, "avail:" + mAvailable + ", ind:" + mCurrentPosition + ", dir:"
