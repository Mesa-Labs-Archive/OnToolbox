...
class SeslRecyclerViewFastScroller {
    ...
    public void setEnabled(boolean enabled) {
        LogUtils.d(TAG, "setEnabled() enabled = " + enabled);
        if (mEnabled != enabled) {
            mEnabled = enabled;
    ...
    private void onStateDependencyChanged(boolean peekIfEnabled) {
        ...
        ViewUtils.resolvePadding(mRecyclerView);
    }
    ...
    private void transitionToHidden() {
        LogUtils.d(TAG, "transitionToHidden() mState = " + mState);
    ...
    private void transitionToVisible() {
        LogUtils.d(TAG, "transitionToVisible()");
        if (mDecorAnimation != null) {
            mDecorAnimation.cancel();
        }
    ...
    private void transitionToDragging() {
        LogUtils.d(TAG, "transitionToDragging()");
        if (mDecorAnimation != null) {
            mDecorAnimation.cancel();
        }
    ...
    private void scrollTo(float position) {
        ...
        LogUtils.d(TAG, "scrollTo() called transitionPreviewLayout() sectionIndex =" + sectionIndex + ", position = " + position);
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getActionMasked()) {
            case MotionEvent.ACTION_DOWN:
                LogUtils.d(TAG, "onInterceptTouchEvent() ACTION_DOWN ev.getY() = " + ev.getY());
            ...
            case MotionEvent.ACTION_MOVE:
                if (!isPointInside(ev.getX(), ev.getY())) {
                    ...
                    scrollTo(pos);
                    LogUtils.d(TAG, "onInterceptTouchEvent() ACTION_MOVE pendingdrag open()");
        ...
        switch (me.getActionMasked()) {
            case MotionEvent.ACTION_DOWN:
                if (isPointInside(me.getX(), me.getY()) && !mRecyclerView.isInScrollingContainer()) {
                    ...
                    LogUtils.d(TAG, "onTouchEvent() ACTION_DOWN.. open() called with posY " + me.getY());
                    return true;
                }
            case MotionEvent.ACTION_UP:
                if (mPendingDrag >= TAP_TIMEOUT) {
                    ...
                    scrollTo(pos);
                    mEffectState = EFFECT_STATE_OPEN;
                    LogUtils.d(TAG, "onTouchEvent() ACTION_UP.. open() called with posY " + me.getY());
                }
                if (mState == STATE_DRAGGING) {
                ...
                break;
            case MotionEvent.ACTION_MOVE:
                LogUtils.d(TAG, "onTouchEvent() ACTION_MOVE.. mState= " + mState + ", mInitialTouchY=" + mInitialTouchY);
                if (mPendingDrag >= TAP_TIMEOUT && Math.abs(me.getY() - mInitialTouchY) > ((float) mScaledTouchSlop)) {
                    ...
                    if (mScrollY > ((float) top) && mScrollY < ((float) bottom)) {
                        LogUtils.d(TAG, "onTouchEvent() ACTION_MOVE 1 mScrollY=" + mScrollY + ", min=" + min + ", max=" + max);
                ...
                if (mState == STATE_DRAGGING) {
                    ...
                    if (mScrollY > ((float) top) && mScrollY < ((float) bottom)) {
                        LogUtils.d(TAG, "onTouchEvent() ACTION_MOVE 2 mScrollY=" + mScrollY + ", min=" + min + ", max=" + max);
                        if (mScrollY < ((float) top) + min) {
                            mScrollY = ((float) top) + min;
                        } else if (mScrollY > max) {
                            mScrollY = max;
